...
config.read(os.getenv('HOME') + '/.omgsm/operators')
gsm_operators = config.get('Main', 'GSMOPERATORS')

#for op in gsm_operators:
#	gsm_op_ = config.getint('Main', 'GSMKRAKENPORT')
...
#		print(gsmpath+"/bin/gsm_convert -s %s/stat/operators/23002.db -f %s/%s %s  | sort -rn | tr \"+\" \"\n\" | cut -d \" \" -f 2- | grep SDCCH/DL"%(gsmpath,newpath,datfile,cflags))
		if os.path.isfile("%s/stat.db"%(gsmsession)):
			statdb="%s/stat.db"%(gsmsession)
		elif os.path.isfile("%s/stat/operators/%s.db"%(gsmpath,mcnc)):
			statdb="%s/stat/operators/%s.db"%(gsmpath,mcnc)
		else:
			statdb="%s/stat/operators/default.db"%gsmpath
		#print("Using %s"%statdb)
		f=os.popen(gsmpath+"/bin/gsm_convert -s %s -f %s/%s %s  | sort -rn | tr \"+\" \"\n\" | cut -d \" \" -f 2- | grep S.CCH/.L"%(statdb,newpath,datfile,cflags))
...
try:
	opts, args = getopt.getopt(sys.argv[1:], "hc:us:r:f:te", ["help", "cflags=","uplink","single=","reusing=","failed-reusing=","realtime","seenfile"])
...
#	con=lite.connect(gsmsession+'keys.db', isolation_level=None)
	...
	# this should not happen, if db is empty, we won't crack anything either
	...
	for i in range(len(result)):
		...
		if not realtime:
			if bursts_to_add > 0:
				...
				break
		else: # running realtime
			if bursts_to_add < 0:
				tsql.execute("delete from keystreams where submitted = -1 order by serial asc limit %i"%-bursts_to_add);
			add_datfile(row[0],row[1],row[2],row[3])
