...
int main(int argc, char* argv[])
{
    ...
    if (argc<4) {
        printf("usage: %s foundkey bitpos framecount (framecount2 burst2 (uplink))\n", argv[0]);
        return -1;
    }

    int streamshift=0; // will be set to 114 for uplink
    ...
    int pos;
    sscanf(argv[2],"%i",&pos);
    pos+=streamshift; // shift 114 bits when processing uplink
    Bidirectional back;
    TheMatrix tm;
    ...
    while (back.PopCandidate(tst)) {
        ...
        if (framecount2>=0) {
            ...
            back.Forwards(mix, 228, bytes);
            int ok = 0;
            for (int bit=streamshift;bit<streamshift+114;bit++) {
                int byte = bit / 8;
                int b = bit & 0x7;
                int v = bytes[byte] & (1<<(7-b));
                char check = v ? '1' : '0';
                if (check==argv[5][bit-streamshift]) ok++; // in uplink, we compare second round of keystream with keystream given
            }
            if (ok>104) {
                printf(" *** MATCHED ***");
            } else {
