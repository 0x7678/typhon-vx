...
//    A5CpuInit(8, 12, 32);
    ...
    if (fd) {
        ...
        // Use all GPUs XXX
	// A5AtiInit(8,12,0x4f,1);
        A5AtiInit(8,12, atimask&0xff,1);
        ...
        for (int i=0; i<samples; i++) {
            ...
            while (it!=mTables.end()) {
                ...
                for (int k=0; k<8; k++) {
                    ...
                    mFragments[fr] = 0;
                    if (mUsingAti) {
		//	printf("ATI Submitting DP search\n");
...
//    }
    ...
    histogram[0] = 0; // wait for distinguished points
    histogram[1] = 0; // wait for disk data (seek)
    histogram[2] = 0; // wait for rest of chain ??? (A5CpuKeySearch -- can be ILX (!))
    histogram[3] = 0; // wait for ???
    int total = 0;
...
//   printf("] state counts (%d): %d %d %d %d\n", total, 
    ...
    printf("SUM: %10d DP(ati): %10d  SE: %10d X(ilx): %10d Y(ati): %10d\n", total, 
...
{
    if (argc<5) {
        printf("usage: %s index_path server_port ilx_mask ati_mask\n", argv[0] );
        return -1;
    }

sscanf(argv[3], "%2hhx", &ilxmask);
sscanf(argv[4], "%2hhx", &atimask);
...
//        usleep(5000); XXX test
        ...
        if (busy) {
            ...
            unsigned long diff = 100000*(dbg2.tv_sec-debug_timer.tv_sec);
