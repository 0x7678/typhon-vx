...
//	int last=0;
	...
	if (dir != NULL) {
		while ((ent = readdir (dir)) != NULL) {
			if(!strncmp("cl-endpoints-", ent->d_name, 12)) {
				sscanf(ent->d_name, "cl-endpoints-%d.bin",&fnum);
				printf("%s %i\n",ent->d_name,fnum);
				found++;
			} else {
				//printf("Not passed: %s \n",ent->d_name);
	...
	if(found) {
		char fname[80];
		sprintf(fname,"cl-endpoints-%09d.bin", fnum);
		char dstf[80];
		sprintf(dstf,"cl/cl-endpoints-%09d.bin", fnum);

		FILE * cfile;
		cfile = fopen(fname, "rb");
		int result = fread (buf,8,16321,cfile);
		fclose (cfile);
		rename(fname, dstf);
		printf("result=%i\n",result);
		uint64_t bnum = buf[0];
		...
		for(int tbl=0; tbl<40; tbl++) {
			for(int i=0; i<408; i++) {
				...
				buf[j] = re;
				//printf("tg: %llx re: %llx j: %i\n", tg,re,j);
			}
		}

		sprintf(fname,"cl-startpoints-%09d.bin", fnum);
		cfile = fopen (fname,"w");
		fwrite (buf , 8, 16321, cfile);
		fclose (cfile);
	} else {
		sleep(1);
	}
